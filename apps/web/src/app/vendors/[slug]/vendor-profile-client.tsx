...
import {
  BadgeCheck, MapPin, Globe, ExternalLink, Link2, X,
  ChevronLeft, Heart, Star, MessageSquare,
} from 'lucide-react'
      ...
      .then(({ data }) => {
        const list: UserEvent[] = Array.isArray(data)
          ? data.map((e: { id: string; title: string; date?: string | null }) => ({
              id: e.id, title: e.title, date: e.date ?? null,
            }))
          : []
        setEvents(list)
    ...
    <div className="fixed inset-0 z-[70] flex items-end sm:items-center justify-center p-4">
      ...
      >
        <div className="flex items-center justify-between">
          ...
          <button type="button" onClick={onClose} aria-label="Close"><X size={16} style={{ color: 'var(--color-muted)' }} /></button>
        </div>
        {done ? (
          <p className="text-sm" style={{ color: 'var(--color-foreground)' }}>Saved to your event mood board.</p>
        ) : events.length === 0 ? (
...
export function VendorProfileClient({
  ...
  useEffect(() => {
    if (!signedIn) return
    proxyClient
      .get<{ favorited: boolean; favoriteCount: number; ownProfile?: boolean }>(`/vendors/${vendor.slug}/favorite-status`)
      .then(({ data }) => {
  ...
  async function toggleFavorite() {
    ...
    try {
      const { data } = next
        ? await proxyClient.post<{ favorited: boolean; favoriteCount: number }>(`/vendors/${vendor.slug}/favorite`)
        : await proxyClient.delete<{ favorited: boolean; favoriteCount: number }>(`/vendors/${vendor.slug}/favorite`)
      if (typeof data?.favoriteCount === 'number') setFavoriteCount(data.favoriteCount)
      setFavorited(Boolean(data?.favorited))
    } catch {
      setFavorited(!next)
  ...
  async function toggleLookLike(post: VendorLook) {
    ...
    setLikeCounts((prev) => ({ ...prev, [post.id]: Math.max(0, (prev[post.id] ?? 0) + (next ? 1 : -1)) }))
    try {
  ...
  useEffect(() => {
    ...
    let cancelled = false
    proxyClient
      .get<{ canReview: boolean; alreadyReviewed: boolean }>(`/vendors/${vendor.slug}/review-status`)
      .then(({ data }) => {
  ...
  const initials = vendor.businessName.split(' ').map((w) => w[0]).join('').toUpperCase().slice(0, 2)
  ...
  return (
    <>
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        ...
        >
          <div className="flex items-start gap-4 flex-wrap sm:flex-nowrap">
            <div
              className="w-16 h-16 rounded-2xl shrink-0 overflow-hidden flex items-center justify-center ring-2 ring-black/8 dark:ring-white/10"
              style={{ background: 'var(--card-bg)' }}
            >
              {vendor.avatarUrl ? (
                // eslint-disable-next-line @next/next/no-img-element
                <img src={vendor.avatarUrl} alt={vendor.businessName} className="w-full h-full object-cover" />
              ) : (
                <span className="text-lg font-bold" style={{ color: 'var(--color-foreground)' }}>{initials}</span>
              )}
            </div>

            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                <h1 className="font-display text-xl font-semibold" style={{ color: 'var(--color-foreground)' }}>
                  {vendor.businessName}
                </h1>
                {vendor.isVerified && (
                  <BadgeCheck size={18} className="text-gold-700 dark:text-gold-400 shrink-0" aria-label="Verified" />
                )}
              </div>
            ...
            <div className="flex items-center gap-2 shrink-0">
              ...
              <button
                ...
                className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium bg-gold-600 text-white hover:bg-gold-700 transition-colors"
              >
          ...
          <div
            className="mt-5 pt-5 border-t grid grid-cols-1 sm:grid-cols-2 gap-4"
            style={{ borderColor: 'var(--color-border)' }}
          >
            {priceLabel && (
              <div>
                <p className="text-xs font-medium mb-0.5" style={{ color: 'var(--color-muted)' }}>Starting price</p>
                <p className="text-sm font-semibold" style={{ color: 'var(--color-foreground)' }}>{priceLabel}</p>
              </div>
            )}
            {vendor.tribesServed.length > 0 && (
              <div>
                <p className="text-xs font-medium mb-0.5" style={{ color: 'var(--color-muted)' }}>Communities served</p>
                <p className="text-sm" style={{ color: 'var(--color-foreground)' }}>{vendor.tribesServed.join(', ')}</p>
              </div>
            )}
          </div>

          {(vendor.websiteUrl || vendor.instagramUrl || vendor.facebookUrl || vendor.externalPortfolioUrl) && (
            ...
            {vendor.posts!.map((item) => (
              <li key={item.id}>
                ...
                >
                  ...
                  <p className="text-xs font-medium px-2 py-1.5 truncate" style={{ color: 'var(--color-foreground)' }}>
                    {item.title}
                  </p>
                </button>
              </li>
      ...
      >
        ...
        {reviews.length === 0 && (
          <p className="text-xs" style={{ color: 'var(--color-muted)' }}>No reviews yet.</p>
        )}
        <ul className="space-y-3">
          {reviews.map((r) => (
            <li key={r.id} className="text-sm">
              ...
              {r.comment && (
                <p className="text-xs mt-0.5" style={{ color: 'var(--color-muted)' }}>{r.comment}</p>
              )}
            </li>
          ))}
        </ul>
        ...
        {reviewStatus?.alreadyReviewed && wantReview && (
          <p className="text-xs pt-3 border-t" style={{ borderColor: 'var(--color-border)', color: 'var(--color-muted)' }}>
            You’ve already reviewed this vendor.
          </p>
        )}
        {reviewStatus && !reviewStatus.canReview && !reviewStatus.alreadyReviewed && !reviewStatus.unauthenticated && wantReview && (
          <p className="text-xs pt-3 border-t" style={{ borderColor: 'var(--color-border)', color: 'var(--color-muted)' }}>
        ...
        <div className="pt-3 border-t space-y-2" style={{ borderColor: 'var(--color-border)' }}>
          ...
          <textarea
            ...
            style={{ background: 'var(--input-bg)', border: '1px solid var(--color-border)', color: 'var(--color-foreground)' }}
          />
          {reviewError && <p className="text-xs" style={{ color: 'var(--color-error, #c45c4a)' }}>{reviewError}</p>}
          <button
            type="button"
            ...
            onClick={() => {
              ...
              startReview(async () => {
                try {
                  const { data } = await proxyClient.post<{ id: string; rating: number; comment: string | null; createdAt: string }>(
                    `/vendors/${vendor.slug}/reviews`,
                    { rating, comment: reviewBody.trim() || undefined },
                  )
                  setReviews((prev) => [{
                    id: data.id,
                    rating: data.rating,
                  ...
                  setReviewError(typeof status?.data?.message === 'string' ? status.data.message : 'Could not submit review')
                }
              })
            }}
      ...
      {showModal && (
        <InquiryModal vendor={vendor} onClose={() => setShowModal(false)} />
      )}
      {savePost && (
        <SaveLookModal
          post={savePost}
