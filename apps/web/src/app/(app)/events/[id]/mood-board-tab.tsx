...
import {
  Sparkles, BookmarkX, MapPin, DollarSign, ChevronRight,
  Loader2, Bookmark, CheckSquare, Receipt, Clock,
} from 'lucide-react'
...
const CATEGORIES = ['ALL', 'PERFORMANCE', 'VENUE', 'DECOR', 'MUSIC', 'FASHION', 'FOOD', 'OTHER'] as const
...
export function MoodBoardTab({ focusEntryId }: { focusEntryId?: string }) {
  ...
  useEffect(() => {
    ...
    const frame = requestAnimationFrame(() => {
      document.getElementById(`mood-entry-${focusEntryId}`)?.scrollIntoView({ behavior: 'smooth', block: 'center' })
    })
  ...
  if (entries.length === 0) {
    return (
      <div className="flex flex-col items-center gap-4 py-16 text-center">
        ...
        <div>
          <p className="font-semibold text-sm" style={{ color: 'var(--color-text-primary)' }}>Your mood board is empty</p>
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h3 className="font-semibold text-sm" style={{ color: 'var(--color-text-primary)' }}>Saved ideas</h3>
          <p className="text-xs mt-0.5" style={{ color: 'var(--color-muted)' }}>
            {entries.length} item{entries.length !== 1 ? 's' : ''} · {Object.keys(counts).length} categories
          </p>
        </div>
        <Link
          href="/inspiration"
          className="inline-flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-xl"
          style={{ background: 'color-mix(in srgb, var(--color-brand-primary) 10%, transparent)', color: 'var(--color-brand-primary)' }}
        >
        ...
        {CATEGORIES.filter((c) => c === 'ALL' || counts[c]).map((cat) => {
          ...
          return (
            <button
              ...
              className="inline-flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-wide px-3 py-1 rounded-full transition-all"
              style={{
                background: active
            ...
            return (
              <div
                ...
                style={{
                  ...
                  border: entry.id === focusEntryId
                    ? '1px solid var(--color-brand-primary)'
                    : '1px solid var(--color-border)',
                }}
              >
                {/* Image */}
                <div className="h-32 flex items-center justify-center relative" style={{ background: cat.bg }}>
                  {item.imageUrl ? (
                  ...
                  <button
                    ...
                    title="Remove"
                  >
                    {removing === item.id ? <Loader2 size={11} className="animate-spin" /> : <BookmarkX size={11} />}
                  </button>
                  )}
                </div>
                ...
                <div className="p-3 flex flex-col gap-2 flex-1">
                  ...
                  {entry.notes && (
                    <p className="text-xs italic px-2 py-1 rounded-lg"
                      style={{ background: 'color-mix(in srgb, var(--color-text-primary) 4%, transparent)', color: 'var(--color-text-secondary)' }}>
                  ...
                  {(entry.checklistItem || entry.budgetItem || (entry.scheduleItems?.length ?? 0) > 0) && (
                    <div className="flex flex-wrap gap-1.5 pt-1">
                      ...
                      {entry.budgetItem && (
                        <span
                          className="flex items-center gap-1 text-[10px] font-medium px-2 py-0.5 rounded-full"
                          style={{ background: 'color-mix(in srgb, #3b82f6 12%, transparent)', color: '#2563eb' }}
                        >
                          <Receipt size={9} /> {entry.budgetItem.label ?? entry.budgetItem.category.replace('_', ' ')}
                        </span>
                      )}
                    </div>
                  )}
                  ...
                  {item.vendorProfile && (
                    <Link
                      ...
                      className="flex items-center justify-between text-xs font-medium px-2.5 py-1.5 rounded-lg"
