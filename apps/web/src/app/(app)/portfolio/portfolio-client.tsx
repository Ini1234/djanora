...
import {
  Check, ExternalLink, ImagePlus, Link2, Plus, Star, Trash2, X,
} from 'lucide-react'
import { proxyClient } from '@/lib/proxy-client'
import type { InspirationCategory, InspirationVisibility, VendorPost, VendorPostTag } from '@/lib/api.types'
...
export function PortfolioClient({
  ...
  useEffect(() => {
    ...
    document.getElementById(`look-${flash.id}`)?.scrollIntoView({ behavior: 'smooth', block: 'nearest' })
    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        ...
        <button
          ...
          className="h-9 px-4 rounded-xl text-sm font-semibold inline-flex items-center gap-2 disabled:opacity-40"
      ...
      >
        ...
        <p className="text-xs" style={{ color: 'var(--color-muted)' }}>
          Optional link out (Instagram, website). Couples can visit it; they cannot save or ask about it.
        </p>
        <div className="grid sm:grid-cols-[1fr_180px_auto] gap-2">
          ...
          <button
            ...
            className="h-9 px-4 rounded-xl text-sm font-medium disabled:opacity-40"
            style={{ background: 'var(--color-brand-primary)', color: '#fff' }}
          >
      ...
      {error && (
        <p className="text-xs" style={{ color: 'var(--color-error, #c45c4a)' }}>{error}</p>
      )}
            ...
            <li id={`look-${post.id}`} key={post.id}>
              <button
                ...
                style={{
                  ...
                  boxShadow: flash?.id === post.id ? '0 0 0 3px color-mix(in srgb, var(--color-brand-primary) 28%, transparent)' : undefined,
                }}
              >
                <div className="aspect-square relative" style={{ background: 'var(--input-bg)' }}>
                  {post.imageUrl ? (
      ...
      {flash && (
        <div
          className="fixed bottom-6 left-1/2 -translate-x-1/2 z-[80] max-w-[calc(100%-2rem)] rounded-2xl px-4 py-3 shadow-2xl flex items-center gap-2"
        ...
        >
          ...
          <p className="text-sm font-medium" style={{ color: 'var(--color-text-primary)' }}>{flash.message}</p>
        </div>
      )}
    </div>
  )
...
function PostEditor({
  ...
  useEffect(() => {
    proxyClient.get<VendorPostTag[]>('/inspiration/tags')
  ...
  async function addLink() {
    ...
    onError('')
    try {
      const { data } = await proxyClient.post<VendorPost>(`/vendors/me/posts/${post.id}/media/link`, {
        url: linkUrl.trim(),
      })
      setLinkUrl('')
      onChange(data)
  ...
  async function setCover(mediaId: string) {
    try {
      const { data } = await proxyClient.patch<VendorPost>(`/vendors/me/posts/${post.id}/media/${mediaId}`)
      onChange(data)
    } catch (err: unknown) {
  ...
  async function removeMedia(mediaId: string) {
    try {
      const { data } = await proxyClient.delete<VendorPost>(`/vendors/me/posts/${post.id}/media/${mediaId}`)
      onChange(data)
    } catch (err: unknown) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4">
      ...
      <div
        className="relative w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-2xl border shadow-2xl p-5 space-y-4"
      ...
      >
        ...
        <label className="block space-y-1">
          <span className="text-xs font-medium" style={{ color: 'var(--color-muted)' }}>Title</span>
          <input
            value={title}
        ...
        <label className="block space-y-1">
          <span className="text-xs font-medium" style={{ color: 'var(--color-muted)' }}>Description</span>
          <textarea
            rows={3}
            value={description}
        ...
        <div className="grid sm:grid-cols-2 gap-3">
          <label className="block space-y-1">
            <span className="text-xs font-medium" style={{ color: 'var(--color-muted)' }}>Category</span>
            <select
              value={category}
          ...
          <label className="block space-y-1">
            <span className="text-xs font-medium" style={{ color: 'var(--color-muted)' }}>Location</span>
            <input
              value={location}
        ...
        <div className="grid sm:grid-cols-3 gap-3">
          <label className="block space-y-1">
            <span className="text-xs font-medium" style={{ color: 'var(--color-muted)' }}>From (CAD)</span>
            <input
              type="number"
              value={from}
          ...
          <label className="block space-y-1">
            <span className="text-xs font-medium" style={{ color: 'var(--color-muted)' }}>To (CAD)</span>
            <input
              type="number"
              value={to}
          ...
          <label className="block space-y-1">
            <span className="text-xs font-medium" style={{ color: 'var(--color-muted)' }}>Cost note</span>
            <input
              value={costNote}
        ...
        <div className="space-y-1">
          <span className="text-xs font-medium" style={{ color: 'var(--color-muted)' }}>Visibility</span>
          <div className="flex flex-wrap gap-1">
            {([
              ...
              <button
                ...
                style={visibility === opt.id
                  ...
                  : { background: 'var(--card-bg)', color: 'var(--color-muted)', border: '1px solid var(--color-border)' }}
              >
                {opt.label}
              </button>
            ))}
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="h-8 px-3 rounded-lg text-xs font-medium inline-flex items-center gap-1 disabled:opacity-40"
              style={{ background: 'var(--color-brand-primary)', color: '#fff' }}
            >
              <ImagePlus size={12} />
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className="h-9 px-3 rounded-xl text-xs font-medium inline-flex items-center gap-1 disabled:opacity-40"
          ...
          <ul className="grid grid-cols-3 sm:grid-cols-4 gap-2">
            {post.media.map((m) => (
              <li key={m.id} className="relative rounded-xl overflow-hidden aspect-square" style={{ background: 'var(--card-bg)' }}>
                {m.mediaType === 'EXTERNAL' ? (
                  ...
                  >
                    ...
                    <span className="text-[10px] line-clamp-2" style={{ color: 'var(--color-muted)' }}>Link</span>
                  </a>
                ) : (
                  // eslint-disable-next-line @next/next/no-img-element
                ...
                {m.isCover && (
                  <span className="absolute top-1 left-1 text-[9px] font-semibold px-1.5 py-0.5 rounded-full bg-black/60 text-white">
                    Cover
                  </span>
                )}
                <div className="absolute inset-x-0 bottom-0 p-1 flex gap-1 bg-gradient-to-t from-black/70 to-transparent">
                  {!m.isCover && (
                    <button
                      type="button"
                      onClick={() => void setCover(m.id)}
                      className="flex-1 h-6 rounded text-[10px] bg-white/90 text-brand-900 inline-flex items-center justify-center gap-0.5"
                    >
                      <Star size={10} /> Cover
                    </button>
                  )}
                  <button
                    ...
                    className="h-6 px-1.5 rounded text-[10px] bg-white/90 text-red-700"
                    aria-label="Remove"
                  >
                    <Trash2 size={10} />
                  </button>
        ...
        <div className="flex items-center justify-between pt-2">
          ...
          <button
            ...
            className="h-9 px-4 rounded-xl text-sm font-semibold disabled:opacity-40 inline-flex items-center gap-1.5"
            style={{ background: 'var(--color-brand-primary)', color: '#fff' }}
          >
            {done ? (
              <>
                <Check size={15} />
                {visibility === 'INSPIRATION' ? 'You’re live' : visibility === 'DRAFT' ? 'Draft saved' : 'Posted'}
              </>
            ) : saving
              ? visibility === 'DRAFT' ? 'Saving…' : 'Posting…'
              : visibility === 'DRAFT' ? 'Save draft'
                : visibility === 'INSPIRATION' ? 'Go live'
                  : 'Post'}
          </button>
        </div>
      </div>
