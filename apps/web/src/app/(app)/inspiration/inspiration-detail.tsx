...
import {
  Bookmark, BookmarkCheck, ChevronLeft, ChevronRight, DollarSign,
  ExternalLink, Heart, Loader2, MapPin, MessageSquare, Send, Users, X,
} from 'lucide-react'
...
export function InspirationDetail({
  ...
  useEffect(() => {
    ...
    proxyClient.get<UserMe>('/users/me')
  ...
  useEffect(() => {
    ...
    backend.get<Comment[]>(`/inspiration/${item.id}/comments`)
  ...
  const media = (detail.media && detail.media.length > 0)
    ? detail.media
    : detail.imageUrl
  ...
  return (
    <>
      <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
        ...
        <div
          className="relative w-full max-w-4xl max-h-[100vh] sm:max-h-[90vh] overflow-hidden sm:rounded-2xl border shadow-2xl flex flex-col sm:flex-row"
          style={{ background: 'var(--color-card)', borderColor: 'var(--color-border)' }}
        >
          <div className="relative sm:w-[52%] aspect-[4/3] sm:aspect-auto sm:min-h-[420px]" style={{ background: 'var(--card-bg)' }}>
            {current?.mediaType === 'EXTERNAL' ? (
              <a href={current.url} target="_blank" rel="noopener noreferrer" className="w-full h-full flex flex-col items-center justify-center gap-2">
                <ExternalLink size={22} style={{ color: 'var(--color-muted)' }} />
                <span className="text-xs" style={{ color: 'var(--color-muted)' }}>Open linked media</span>
              </a>
            ) : current ? (
              // eslint-disable-next-line @next/next/no-img-element
              <img src={current.url} alt={detail.title} className="w-full h-full object-cover" />
            ) : (
              <div className="w-full h-full flex items-center justify-center text-sm" style={{ color: 'var(--color-muted)' }}>
                No photo
              </div>
            )}
            {media.length > 1 && (
            ...
            <button
              type="button"
              onClick={onClose}
              className="absolute top-2 right-2 w-8 h-8 rounded-full bg-black/50 text-white flex items-center justify-center sm:hidden"
              aria-label="Close"
            >
              <X size={16} />
            </button>
          ...
          <div className="flex-1 flex flex-col min-h-0 max-h-[55vh] sm:max-h-[90vh]">
            <div className="flex items-start justify-between gap-3 px-5 py-4 border-b" style={{ borderColor: 'var(--color-border)' }}>
              <div className="min-w-0">
                <p className="text-[10px] font-semibold uppercase tracking-wider" style={{ color: 'var(--color-brand-primary)' }}>
                  {detail.category}
                </p>
              ...
              <button type="button" onClick={onClose} className="hidden sm:flex p-1.5" aria-label="Close">
            ...
            <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
              ...
              {((detail.tagItems && detail.tagItems.length > 0) || detail.tags.length > 0) && (
                <div className="flex flex-wrap gap-1">
                  {(detail.tagItems ?? detail.tags.map((label) => ({ slug: label, label }))).map((tag) => (
                    <span
                      key={tag.slug}
              ...
              <div className="flex flex-wrap gap-3 text-[12px]" style={{ color: 'var(--color-muted)' }}>
                {(detail.location || detail.vendorProfile?.city) && (
                  <span className="inline-flex items-center gap-1"><MapPin size={11} /> {detail.location ?? detail.vendorProfile?.city}</span>
                )}
                {(detail.saveCount ?? 0) > 0 && (
                  <span>Saved by {detail.saveCount}</span>
                )}
                {detail.priceRangeFrom != null && (
                  <span className="inline-flex items-center gap-1">
                    <DollarSign size={11} />
                    {detail.currency}{detail.priceRangeFrom.toLocaleString()}
                    {detail.priceRangeTo ? `–${detail.priceRangeTo.toLocaleString()}` : '+'}
                  </span>
                )}
              </div>
              {detail.costNote && (
                <p className="text-xs" style={{ color: 'var(--color-muted)' }}>{detail.costNote}</p>
              )}
              ...
              <div className="pt-2 border-t space-y-3" style={{ borderColor: 'var(--color-border)' }}>
                ...
                {loadingComments ? (
                  ...
                  <p className="text-xs" style={{ color: 'var(--color-muted)' }}>No comments yet. Ask a question or share a thought.</p>
                ) : (
                  <ul className="space-y-3">
                    {comments.map((c) => (
                      <li key={c.id} className="flex gap-2">
                        <div
                          className="w-7 h-7 rounded-full shrink-0 overflow-hidden flex items-center justify-center text-[9px] font-bold"
                          style={{ background: 'var(--card-bg)', color: 'var(--color-muted)' }}
                        >
                          {c.author.avatarUrl ? (
                            // eslint-disable-next-line @next/next/no-img-element
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="flex items-baseline justify-between gap-2">
                            <p className="text-xs font-medium" style={{ color: 'var(--color-text-primary)' }}>{authorName(c)}</p>
                            {me?.id === c.author.id && (
                              <button
                                type="button"
                          ...
                          <p className="text-xs leading-relaxed whitespace-pre-wrap" style={{ color: 'var(--color-text-secondary)' }}>{c.body}</p>
                        </div>
                      </li>
                    ))}
                  </ul>
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className="h-9 px-3 rounded-xl text-sm font-medium disabled:opacity-40"
                  ...
                  <button
                    ...
                    style={{ background: 'var(--card-bg)', border: '1px solid var(--color-border)', color: 'var(--color-foreground)' }}
                  >
                    Sign in to comment
                  </button>
                )}
            ...
            <div className="px-5 py-3 border-t flex flex-wrap gap-2" style={{ borderColor: 'var(--color-border)' }}>
              <button
                type="button"
                onClick={() => requireAuth(() => onLikeClick?.())}
                className="h-9 px-3 rounded-xl text-xs font-semibold inline-flex items-center gap-1"
                style={{
              ...
              <button
                ...
                className="h-9 px-3 rounded-xl text-xs font-semibold inline-flex items-center gap-1"
                style={{ background: saved ? 'var(--color-brand-primary)' : 'var(--card-bg)', color: saved ? '#fff' : 'var(--color-foreground)', border: saved ? 'none' : '1px solid var(--color-border)' }}
              >
